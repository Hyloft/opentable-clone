const displayTimeObject:{[key:string]:string} = {
  ...
  export const convertToDisplayTime = (time: string) => {
    const t = Object.keys(displayTimeObject).find(t=>t==time)
    if(!t) return ''
    const idx = Object.keys(displayTimeObject).findIndex(t=>t==time)
    return Object.values(displayTimeObject)[idx]
  }